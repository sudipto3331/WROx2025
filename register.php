...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Optional file uploads → JSON paths
  $docPaths = [];
  if (!empty($_FILES['docs']) && is_array($_FILES['docs']['name'])) {
    ...
    for ($i=0; $i<count($_FILES['docs']['name']); $i++) {
      ...
      $ext = strtolower(pathinfo($_FILES['docs']['name'][$i], PATHINFO_EXTENSION) ?: 'bin');
      $name = 'doc_'.date('YmdHis').'_' . bin2hex(random_bytes(4)) . '.' . $ext;
      $dest = UPLOAD_DIR . '/' . $name;
      if (!move_uploaded_file($_FILES['docs']['tmp_name'][$i], $dest)) { $errors[] = 'Failed to save document #'.($i+1); continue; }
  ...
  if (empty($errors)) {
    ...
    $html      = email_verify_template($first_name ?: $username, $verifyUrl);
