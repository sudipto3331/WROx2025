...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $pdo = db();
  $stmt = $pdo->prepare("SELECT id, username, first_name FROM users WHERE email = :e LIMIT 1");
  ...
  if ($user) {
    ...
    $html     = email_reset_template($user['first_name'] ?: $user['username'], $resetUrl);
    send_mail_html($email, 'Reset your password - SORA Labs', $html);
  }
